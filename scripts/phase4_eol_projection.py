...
annual_forecast = forecast_df.groupby('Year')['Forecast_Registrations'].sum().reset_index()
...
ax.stackplot(eol_df['Year'], 
             [eol_df[cls] for cls in classes],
             labels=classes,
             colors=colors,
             alpha=0.85)

ax.set_title('Projected End-of-Life EV Batteries in India (2020–2035)', fontsize=14, fontweight='bold')
...
for yr in [2030, 2035]:
    ...
    ax.annotate(f'{total:,.0f}', xy=(yr, total), 
