...
annual_forecast = forecast_df.groupby('Year').agg({
    'yhat_lower': 'sum',
    'Forecast_Registrations': 'sum',
    'yhat_upper': 'sum'
}).reset_index()
...
iea_bev = iea_df[(iea_df['powertrain'] == 'BEV') & 
                  (iea_df['mode'].isin(['2 and 3 wheelers', 'Cars', 'Buses', 'Vans', 'Trucks']))]
iea_recent = iea_bev[(iea_bev['year'] >= 2020) & (iea_bev['year'] <= 2025)]
class_shares = (iea_recent.groupby('mode')['value'].sum() / iea_recent['value'].sum()).to_dict()
...
annual_chem = chem_df.groupby('Year').first().reset_index()[['Year', 'LFP_Share', 'NMC_Share']]
...
for sc_name, sc_info in scenarios.items():
    ...
    for cls, share in class_shares.items():
        ...
        for sale_year in annual_forecast['Year']:
            ...
            total_sales = annual_forecast.loc[annual_forecast['Year'] == sale_year, forecast_col].values[0]
...
smip_df = master_df.groupby(['Year', 'Scenario', 'Probability', 'Vehicle_Class', 'Chemistry'])['EOL_Volume'].sum().reset_index()
...
print("Saved SMIP master scenario dataset to data/processed/smip_feedstock_scenarios.csv")
...
ax.plot(plot_df.index, plot_df['Base'], label='Base Scenario (50% prob)', color='blue', linewidth=3)
ax.fill_between(plot_df.index, plot_df['Pessimistic'], plot_df['Optimistic'], 
                color='lightblue', alpha=0.4, label='Pessimistic - Optimistic Bounds (25% prob each)')

ax.set_title('EV Battery EOL Volume Scenarios (2020-2035)', fontsize=14, fontweight='bold')
