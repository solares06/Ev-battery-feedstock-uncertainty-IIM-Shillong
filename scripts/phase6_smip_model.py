...
zone_shares = {'North': 0.25, 'South': 0.35, 'East': 0.10, 'West': 0.20, 'Central': 0.10}

# Potential Recycling Facilities (J)
facilities = ['Delhi_NCR', 'Chennai_Hub', 'Pune_Hub']

# Scenarios (S) and Probabilities
# Probabilities match our Phase 5 assumptions
...
for s in scenarios:
    for c in chemistries:
        ...
        vol = df_2030[(df_2030['Scenario'] == s) & (df_2030['Chemistry'] == c)]['EOL_Volume'].sum()
        for i in zones:
...
x = pulp.LpVariable.dicts("Flow", 
                          [(i, j, s, c) for i in zones for j in facilities for s in scenarios for c in chemistries], 
                          lowBound=0, cat='Continuous')

# z_is^c: Flow lost to informal sector from zone i under scenario s for chemistry c
z = pulp.LpVariable.dicts("Informal_Loss", 
...
first_stage_cost = pulp.lpSum([fixed_cost[j] * y[j] + unit_cap_cost * cap[j] for j in facilities])

# Second-stage cost (Expected across scenarios)
expected_second_stage_cost = pulp.lpSum(
    scen_probs[s] * (
        # Transport Costs
        pulp.lpSum(trans_cost[i][j] * x[(i, j, s, c)] for i in zones for j in facilities for c in chemistries)
        ...
        - pulp.lpSum(revenue[c] * x[(i, j, s, c)] for i in zones for j in facilities for c in chemistries)
    )
    for s in scenarios
)
...
with open('outputs/smip_results.txt', 'w') as f:
    ...
    for j in facilities:
        if pulp.value(y[j]) > 0.5:
            f.write(f"[OPEN] Facility: {j} | Designed Capacity: {pulp.value(cap[j]):,.0f} batteries\n")
        else:
    ...
    for s in scenarios:
        ...
        total_formal = sum(pulp.value(x[(i, j, s, c)]) for i in zones for j in facilities for c in chemistries)
        total_informal = sum(pulp.value(z[(i, s, c)]) for i in zones for c in chemistries)
