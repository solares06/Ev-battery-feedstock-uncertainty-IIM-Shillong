...
results_lines.append(
    f"Objective Value (Expected Cost): ₹{pulp.value(model.objective):,.0f}"
)
...
for s in scenario_list:
    ...
    total_informal = sum(
        pulp.value(z[(i, s, c)]) for i in supply_regions for c in chemistries
    )
    results_lines.append(f"    Formal Recycling:     {total_formal:>15,.0f} batteries")
    results_lines.append(
        f"    Informal Sector Loss: {total_informal:>15,.0f} batteries"
    )
    results_lines.append(
        f"    Informal Loss Rate:   {total_informal / (total_formal + total_informal) * 100:.1f}%"
    )
    ...
    for state, fac, flow in flows[:8]:
        results_lines.append(
            f"      {state_short[state]:>2} → {fac:<14}: {flow:>12,.0f}"
        )
...
bars = ax.bar(
    opened_facs.keys(),
    [v / 1e6 for v in opened_facs.values()],
    color=colors[: len(opened_facs)],
)
ax.set_title(
    "Optimal Recycling Facility Capacities (2030)", fontsize=14, fontweight="bold"
)
