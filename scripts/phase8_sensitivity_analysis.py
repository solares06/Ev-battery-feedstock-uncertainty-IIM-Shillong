...
def solve_smip(
    ...
    expected_recourse = pulp.lpSum(
        ...
        * (
            ...
            + pulp.lpSum(
                penalty_cost * z[(i, s, c)] for i in supply_regions for c in chemistries
            )
            - pulp.lpSum(
                revenue[c] * x[(i, j, s, c)]
                for i in supply_regions
    ...
    for j in facilities:
        for s in scenario_list:
            model += (
                pulp.lpSum(x[(i, j, s, c)] for i in supply_regions for c in chemistries)
                <= cap[j]
            )
    ...
    base_informal = sum(
        pulp.value(z[(i, scenario_list[0], c)])
        for i in supply_regions
        for c in chemistries
    )
    informal_pct = (
        base_informal / (total_supply / len(chemistries) if total_supply > 0 else 1)
    ) * 100
...
base_fixed_cost = {
    "Delhi_NCR": 40_00_00_000,
    "Chennai": 35_00_00_000,
    "Pune": 38_00_00_000,
    "Hyderabad": 32_00_00_000,
    "Ahmedabad": 36_00_00_000,
    "Kolkata": 30_00_00_000,
}
base_unit_cap = 400
base_penalty = 2000
...
base_supply = {}
total_volM = df_2030.groupby("Scenario")["EOL_Volume"].sum().max()
for s in scen_probs:
    for c in chemistries:
        total_vol = df_2030[(df_2030["Scenario"] == s) & (df_2030["Chemistry"] == c)][
            "EOL_Volume"
        ].sum()
        for state in supply_regions:
...
for nmc_rev in nmc_values:
    ...
    if res:
        nmc_results.append(
            {
                "NMC_Revenue": nmc_rev,
                "Obj_Crore": res["obj"] / 1e7,
                "Facilities_Open": res["n_open"],
                "Opened": ", ".join(res["opened"]),
                "Informal_Loss_Pct": res["informal_pct"],
            }
        )
        print(
            f"  NMC ₹{nmc_rev}: Obj=₹{res['obj']/1e7:.0f}Cr, Open={res['n_open']}, Informal={res['informal_pct']:.1f}%"
        )
...
for pen in penalty_values:
    ...
    if res:
        ...
        print(
            f"  Penalty ₹{pen}: Obj=₹{res['obj']/1e7:.0f}Cr, Open={res['n_open']}, Informal={res['informal_pct']:.1f}%"
        )
...
for mult in fixed_multipliers:
    ...
    if res:
        ...
        print(
            f"  Fixed Cost x{mult}: Obj=₹{res['obj']/1e7:.0f}Cr, Open={res['n_open']}, Informal={res['informal_pct']:.1f}%"
        )
...
for mult in trans_multipliers:
    ...
    for state in supply_regions:
        scaled_trans[state] = {
            fac: base_trans_cost[state][fac] * mult for fac in facilities
        }
    res = solve_smip(
        supply_regions,
        region_shares,
    ...
    if res:
        ...
        print(
            f"  Transport x{mult}: Obj=₹{res['obj']/1e7:.0f}Cr, Open={res['n_open']}, Informal={res['informal_pct']:.1f}%"
        )
...
fig.suptitle(
    "Sensitivity Analysis of SMIP Model Parameters", fontsize=16, fontweight="bold"
)
