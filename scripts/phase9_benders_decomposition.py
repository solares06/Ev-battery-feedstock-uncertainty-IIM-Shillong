...
# Big-M for capacity
M_val = df_2030.groupby("Scenario")["EOL_Volume"].sum().max() * 2
...
def solve_master(cuts, iteration):
    ...
    master += (
        pulp.lpSum([fixed_cost[j] * y[j] + unit_cap_cost * cap[j] for j in facilities])
        + theta,
        "Master_Obj",
    )
...
def solve_subproblem(y_fixed, cap_fixed, scenario):
    ...
    sub += (
        pulp.lpSum(
            trans_cost[i][j] * x[(i, j, c)]
            for i in supply_regions
            for j in facilities
            for c in chemistries
        )
        + pulp.lpSum(
            penalty_cost * z[(i, c)] for i in supply_regions for c in chemistries
        )
        - pulp.lpSum(
            revenue[c] * x[(i, j, c)]
            for i in supply_regions
    ...
    for i in supply_regions:
        for c in chemistries:
            ...
            sub += (
                pulp.lpSum(x[(i, j, c)] for j in facilities) + z[(i, c)]
                == supply_data[(i, s, c)],
                safe_name,
            )
    ...
    for j in facilities:
        safe_name = f"Cap_{j}"
        sub += (
            pulp.lpSum(x[(i, j, c)] for i in supply_regions for c in chemistries)
            <= cap_fixed[j],
            safe_name,
        )
    ...
    for i in supply_regions:
        for c in chemistries:
            safe_name = f"Supply_{i.replace(' ', '_')}_{c}"
            pi_vals[(i, c)] = (
                sub.constraints[safe_name].pi
                if sub.constraints[safe_name].pi is not None
                else 0
            )
...
for iteration in range(1, MAX_ITER + 1):
    ...
    first_stage_cost = sum(
        fixed_cost[j] * y_val[j] + unit_cap_cost * cap_val[j] for j in facilities
    )
    ...
    print(
        f"{iteration:>4} | {lower_bound/1e7:>18,.2f} | {upper_bound/1e7:>18,.2f} | {gap:>9.4f}% | {len(cuts):>5}"
    )
...
results_lines.append(
    f"Optimal Objective: ₹{upper_bound:,.0f} (₹{upper_bound/1e7:.2f} Crore)"
)
...
results_lines.append(
    f"{'Iter':>4} | {'LB (₹ Cr)':>12} | {'UB (₹ Cr)':>12} | {'Gap (%)':>10}"
)
for row in convergence_log:
    results_lines.append(
        f"{row['Iteration']:>4} | {row['Lower_Bound']:>12,.2f} | {row['Upper_Bound']:>12,.2f} | {row['Gap_Pct']:>9.4f}%"
    )
