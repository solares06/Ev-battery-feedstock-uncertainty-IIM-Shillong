...
future = m.make_future_dataframe(periods=12*10, freq='MS') # 10 years from Dec 2025 -> ~2035
# Merge future with full policy dummy dataframe
future_with_regressors = pd.merge(future, policy_df, left_on='ds', right_on='Date', how='left')
...
# Format output
forecast_out = forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].rename(columns={'ds': 'Date', 'yhat': 'Forecast_Registrations'})
...
# 5. Plotting
fig = m.plot(forecast)
plt.scatter(test_df['ds'], test_df['y'], color='red', label='Holdout Test Data', zorder=5)
